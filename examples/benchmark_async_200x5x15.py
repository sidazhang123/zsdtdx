"""
async-only 基准脚本：200 code * 5 freq * 15 samples。

用途：
1. 仅运行 async 档位，支持你在优化 async 时快速复测。
2. 输出与完整对比脚本一致口径的统计字段，便于和已有 sync 结果手工对比。
"""
...
def _build_meta(
    *,
    ...
    2. samples: 时间样本列表。
    3. tasks: 全量任务列表。
    4. profile: 本次 async 生效配置快照。
    输出：
    1. 元数据字典。
    用途：
    1. 保持与完整脚本的元数据结构尽量一致。
    边界条件：
    1. 仅包含 async 档位，不包含 sync 档位。
    """
    return {
...
def main() -> None:
    """
    运行 async-only 基准并写出结果文件。

    输入：
    1. 无显式输入参数。
    输出：
    1. 控制台进度日志 + `benchmark_async_200x5x15_result.json`。
    用途：
    1. 在 async 调优阶段快速验证吞吐与稳定性，不重复执行 sync。
    边界条件：
    ...
    print("=" * 96)
    print("get_stock_kline async-only benchmark (200*5*15)")
    print("=" * 96)
    ...
    out_path = Path(__file__).with_name(
        f"benchmark_async_{int(len(codes))}x{int(len(bench.FREQS))}x{int(len(samples))}_result.json"
    )
...
if __name__ == "__main__":
    try:
        ...
        bench._cleanup_parallel_pool(run_name="benchmark-async-exit")
