"""
生成固定 benchmark 任务集并落盘：200 code * 5 freq * 15 samples。

用途：
1. 将任务样本固定为文件，避免每次基准运行时重新随机抽样。
2. 供 sync-only / async-only 基准脚本读取同一份任务集，确保口径一致。
"""
...
def _taskset_hash(tasks: List[Dict[str, str]]) -> str:
    ...
    1. 任务集哈希字符串。
    用途：
    1. 便于后续验证 sync/async 是否读取到同一份任务集。
    边界条件：
    1. 空列表仍会返回固定哈希值。
    """
...
def main() -> None:
    ...
    1. 无显式输入参数。
    输出：
    1. `benchmark_taskset_200x5x15.json`。
    用途：
    1. 固定 benchmark 输入任务集，供后续单独跑 sync/async。
    边界条件：
    1. 写文件失败时会抛出异常。
    """
    codes = bench._collect_stock_codes(bench.TARGET_STOCK_COUNT)
    samples: List[Tuple[str, str]] = bench._build_samples(bench.SAMPLE_COUNT, seed=SAMPLE_SEED)
    tasks = bench.build_tasks(codes, bench.FREQS, samples)
    task_hash = _taskset_hash(tasks)

    report: Dict[str, Any] = {
        "meta": {
            "taskset_name": "benchmark_taskset_200x5x15",
            ...
            "generator_script": "examples/generate_benchmark_taskset_200x5x15.py",
        },
        "codes": list(codes),
