...
职责：
1. 构造标准行情连接后的三条握手请求。
2. 对齐通达信官方客户端（TdxW）在 7709 上的握手字节，而不是旧 pytdx 身份包。

边界：
1. 仅负责组包与丢弃握手回包，不解析业务字段。
2. 第一条 292 字节包的末 8 字节按连接随机生成；前 284 字节为官方客户端常量。
"""
...
class SetupCmd3(BaseParser):
    def setup(self):
        ...
        边界：尾字段固定为 7.73 / 05，与当前 TdxW 样本一致。
        """
